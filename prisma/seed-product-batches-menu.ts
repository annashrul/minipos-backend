...
// Menu untuk fitur traceability batch/lot + FEFO + product recall.
const MENU = {
  key: "product-batches",
  name: "Batch & Kedaluwarsa",
  ...
  actions: [
    ...
    { key: "update", name: "Edit", sortOrder: 1 },
    { key: "recall", name: "Product Recall", sortOrder: 2 },
  ],
};
...
async function main() {
  // Upsert menu (idempotent — aman dijalankan berulang).
  ...
  // Grant menu + semua action ke role.
  ...
  console.log(
    `Menu '${MENU.key}' siap (akses penuh: ${ROLES.join(", ")}).`,
  );
}
