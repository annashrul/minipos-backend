...
const MENU_SEED = [
  ...
  },
  {
    ...
    actions: [
      ...
      "delete",
      "export",
      // Generate utility — pre-fill kode produk & barcode unik di form.
      // Default: dianggap bagian dari "update" (admin/manager dapat by default).
      "generate_code",
      "generate_barcode",
    ],
  },
  ...
  for (const menu of MENU_SEED) {
    const createdMenu = await prisma.appMenu.create({
      data: {
        ...
        path: menu.path,
        group: menu.group,
        ...((menu as { subgroup?: string }).subgroup
          ? { subgroup: (menu as { subgroup?: string }).subgroup ?? null }
          : {}),
